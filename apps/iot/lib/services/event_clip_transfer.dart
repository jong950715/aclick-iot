...
class EventTransferModel {
  ...
  EventTransferModel({required this.state, required this.filename});

  EventTransferModel copyWith({EventTransferState? state, String? filePath}) {
...
class EventClipTransfer extends ListNotifier<EventTransferModel, String> {
  ...
  Future<void> initialize() async {
    ref
        ...
        .listen((filename) {
          upsert(EventTransferModel(filename: filename, state: EventTransferState.completed));
    });
    return;
  }
